...
def scaled_dot_product_attention(q, k, v, mask):        # autoreg mask padding of q,k,v is already done here
...
class MultiHeadAttention(tf.keras.layers.Layer):             # So this is a multihead attention layer superclass is the Keras layer here 
    ...
    def call(self, v, k, q, query_mask,key_mask,value_mask):                # Forward pass here ::  The mask here is the padding mask 
        ...
        if(self.autoreg):
            autoreg_mask = self.create_autoreg_mask(batch_size,self.num_heads,query_len,key_len)
        else:
            autoreg_mask = None
            
#         print(autoreg_mask.shape)
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self,d_model, num_heads, dff, rate=0.1,autoregressive=False):
        super(EncoderLayer, self).__init__()

        self.mha = MultiHeadAttention(d_model, num_heads,autoreg=autoregressive)
