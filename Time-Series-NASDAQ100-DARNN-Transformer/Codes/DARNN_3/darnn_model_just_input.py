...
"""
using simply the input as it is, meaning a single attention RNN  :: This is to experimentally test out the need for dual attention here : 
"""

class InputAttentionEncoder_modified(nn.Module):          # No weighting of the Inputs is done here :: 
...
class TemporalAttentionDecoder(nn.Module):
    def __init__(self, M, P, T, stateful=False):
        ...
        :param: P:
            number of deocder LSTM units            # Again single decoder layer here 
        :param: T:
            number of timesteps           
        :param: stateful:
...
class DARNN_just_input(nn.Module):
    def __init__(self, N, M, P, T, stateful_encoder=False, stateful_decoder=False):
        super(self.__class__, self).__init__()
        self.encoder = InputAttentionEncoder_modified(N, M, T, stateful_encoder).cuda()
...
model = DARNN_just_input(81, 64, 64, 16).cuda()         # self, N, M, P, T, stateful_encoder=False, stateful_decoder=False
